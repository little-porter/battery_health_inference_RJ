...
void rs485_driver_init(rs485_driver_t *rs485_drv)
{
    ...
    io_conf.intr_type = GPIO_INTR_DISABLE; // 绂佺敤涓?鏂?
    io_conf.mode = GPIO_MODE_OUTPUT; // 璁剧疆涓鸿緭鍑烘ā寮?
    io_conf.pin_bit_mask = (1ULL << RS485_EN_PIN); // 閫夋嫨鍏蜂綋鐨凣PIO
    io_conf.pull_down_en = GPIO_PULLDOWN_DISABLE; // 绂佺敤涓嬫媺鐢甸樆
    io_conf.pull_up_en = GPIO_PULLUP_DISABLE; // 绂佺敤涓婃媺鐢甸樆
    gpio_config(&io_conf);
