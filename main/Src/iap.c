...
uint16_t   iap_delay_time = IAP_TIMEOUT;					//IAP过程超时时间
...
bool iap_bin_crc(void)
{
    ...
    fseek(file, 0L, SEEK_END);  // 将位置移到文件结尾
...
bool iap_app_update(void)
{
	...
	fseek(file_cache, 0L, SEEK_END);  // 将位置移到文件结尾
...
void iap_msg_deal_handler(uint8_t *data,uint16_t length)
{
	...
	if(data_num <= 1024){
		iap_write_data(&data[8],data_num,500);		//灏嗘枃浠跺彂閫佸埌iap
	}else{;}
}
